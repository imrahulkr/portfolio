...
- Process section redesigned (2026-09-26) from four plain cards to a winding path (`components/home/process.tsx`, now a client component): on desktop a wavy dashed SVG path with an accent line that draws left to right on scroll, four numbered nodes alternating high and low, stems down to a clean row of cards; on mobile a curved connector winds down the left edge. Hover fills the node, glows the card and lights the stem (covered by the approved card-hover glow exception). Path geometry constants are documented at the top of the file; the wave's node x positions assume four equal columns, so adding or removing a step means updating them.

- About section (2026-09-26): the three `[Add ...]` placeholders are gone. "Currently building" now shows the in-progress project (FluxGate, found via `status: "in-progress"` in `data/projects.ts`) with its title, a one-line summary in `aboutContent.currentlyBuilding`, real tech chips and a case-study link. The other two columns were replaced, at the owner's request, with sections built only from existing real data: Recognition (INSTA award from `data/stats.ts` plus `data/achievements.ts`) and Problem solving (DSA count from `data/stats.ts` plus the LeetCode link). `stats[1]` and `stats[2]` are read by index, so reordering `data/stats.ts` would change this section. `AboutContent` no longer has `currentlyLearning`/`interestedIn`; the resume page only uses `intro`.
